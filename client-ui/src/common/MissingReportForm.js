...
const MissingReportForm = ({ activityId, setShow }) => {
  ...
  const submitConfirmJoin = async (e) => {
    ...
    try {
      ...
      let res = await authApi().get(
        endpoints["getExistMissingReport"](activityId)
      );
      if (res.data.result === null) {
        await authApi().post(endpoints["createMissingReport"], formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
        setLoading(false);
        navigate("/mising-report");
      }
      else{
        alert("da ton tai!")
      }
    ...
    <div className="w-full h-full fixed z-50 flex justify-center left-0 top-14">
      <form onSubmit={submitConfirmJoin} className="w-1/2 max-h-xl">
        <div class="items-start p-12">
          <div class="mx-auto w-full bg-white shadow-xl rounded-md">
