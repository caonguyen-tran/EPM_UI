...
const Comment = ({ ...props }) => {
  ...
  useEffect(() => {
    const loadComments = async () => {
      try {
        setLoading(true);
        let res = await API.get(endpoints["activityComments"](props.id));
    ...
    comments.map((item) => (
      <div class="flex-col w-full py-4 mx-auto bg-white border-b-2 border-r-2 border-gray-200 shadow-md px-3 mb-5">
        <div class="flex flex-row relative">
          ...
          {item.isParent && !show ? (
            <button onClick={() => showSubComment(item.id)} className="font-medium text-center min-h-10 min-w-36 absolute text-white bg-blue-700 hover:bg-blue-800 top-full left-3/4 rounded shadow-md">
              Xem thêm...
            </button>
          ) : (
            <></>
